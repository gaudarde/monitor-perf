...
if __name__ == '__main__':
    ...
    '''    if datetime.fromtimestamp(os.path.getatime('data/table.csv')).date() < datetime.today().date():
        download.download()
        download.merge()
    else:
        pass'''
    ...
    dfData.to_csv('data/data.csv', index=False, encoding='cp1252', decimal=',')
    dfData.to_csv('assets/data.csv', index=False, encoding='cp1252', decimal=',')
    print(f'df data.csv (todas as colunas)\n'
          f'{dfData.head()}\n')

    # Dados para a tabela (data.csv)
    dfTable = df.iloc[:, [7, 8, 0, 17, 3, 4, 10, 5, 15, 11, 16]]
    dfTable.to_csv('data/table.csv', index=False, encoding='cp1252', decimal=',')
